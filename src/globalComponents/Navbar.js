...
const Navbar = () => {
  ...
  useEffect(() => {
    db.collection("admin")
      .get()
      .then((collections) => {
        const newData = collections.docs
        .map((doc) => ({...doc.data(), id:doc.id }));
        setadminuser(newData);                
        console.log(adminuser, newData);
      })
  })
  ...
  return (
    <header>
      {/* ......Mini navbar start...... */}
      <div className="py-3 bg-[#98d0e2] hidden sm:block">
      ...
      >
        <div className="flex justify-between items-center container mx-auto px-5">
          ...
          <div>
            ...
            >
              <h1 className={"font-bold text-[#ed4249] font-size 30px  md:text-3xl font-bold  "}>Aries Job portal</h1>
            </NavLink>
          </div>
